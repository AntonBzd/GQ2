...
First stage of the copula pipeline.

References:
    - Glosten, Jagannathan & Runkle (1993), Journal of Finance
    - Hansen (1994), International Economic Review
    - Rensen (2025), Section 2.2 + Appendix C
    - Oh & Patton (2023), Section 5.1
"""
...
class HansenSkewT:
    """
    Standardized skewed Student-t (Hansen 1994).
...
# ---------------------------------------------------------------------------
# AR(1) + GJR-GARCH(1,1)   —   Gaussian quasi-MLE
# ---------------------------------------------------------------------------
...
def fit_all(
    ...
    for i, ticker in enumerate(log_rets.columns):
        ...
        if verbose:
            ...
            print(f"  α={p.alpha:.3f}  κ={p.kappa:.3f}  β={p.beta:.3f}"
                  f"  η={p.eta:.1f}  λ={p.lam:+.3f}")
    return results
...
def summary_table(results: dict[str, MarginalResult]) -> pd.DataFrame:
    """Cross-sectional summary matching Rensen Appendix C, Table 3 (Panel B)."""
    rows = []
