...
def _block_corr(S: np.ndarray, gamma: np.ndarray, G: int) -> np.ndarray:
    """GÃ—G matrix of average correlations """
    Omega = np.zeros((G, G))
...
def _extract_loadings(Omega: np.ndarray) -> FactorLoadings:
    """
    Variance targeting: extract loadings from correlations via eigendecomposition.
    """
...
def _distance_matrix(
    ...
    """
    NÃ—G matrix, Measures how well variable i fits into group g.
    """
    N = S.shape[0]
    D = np.zeros((N, G))
...
def _reassign(
    ...
    """k-means move each variable to its closest group."""
...
def _build_R(gamma: np.ndarray, loadings: FactorLoadings) -> np.ndarray:
    """NÃ—N model-implied correlation matrix from factor structure"""
    lM, lC = loadings.lambda_M, loadings.lambda_C
...
def fit_static_clusters(
    ...
    Inputs
    ----------
    U : (T, N) uniform probabilities values from marginal model
    G : number of clusters
    n_starts : random restarts , can take 10 as Oh & Patton 2023
    ...
    if 2 * G > N:
        raise ValueError(f"Need N â‰¥ 2G for identification. N={N}, G={G}.")
...
def select_n_groups(
    ...
    if verbose:
        print(f"\n  â†’ Optimal G = {best_G}  (BIC = {results[best_G].bic:.0f})")
...
def cluster_table(result: ClusterResult) -> pd.DataFrame:
    ...
    for g in range(result.n_groups):
        ...
        rows.append({
            "Group": g + 1,
            "Size": len(members),
            "Lambda_M": f"{result.loadings.lambda_M[g]:.3f}",
            "Lambda_C": f"{result.loadings.lambda_C[g]:.3f}",
